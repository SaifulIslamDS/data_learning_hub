...
def render_teaching_section(item: dict, n: int) -> str:
    ...
    code = ''
    if item.get('code'):
        code = f'<div class="formula-block"><span>Excel</span><code>{escape(item["code"])}</code><button type="button" class="copy-code" data-copy-code aria-label="Copy formula">Copy</button></div>'
