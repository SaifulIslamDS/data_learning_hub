...
def render_chapter(tutorial: dict, chapter: dict, index: int, shell) -> str:
    ...
    extra = '<script src="/assets/js/tutorial-core.js" defer></script>' + ('<script src="/assets/js/sql-practice.js" defer></script>' if tutorial['id']=='sql-data-analytics' else '') + '<script src="/assets/js/tutorial.js" defer></script>'
...
def render_tutorials_index(tutorials: list[dict], domains: list[dict], shell) -> str:
    ...
    upcoming = []
    for d in domains:
        tutorial_id = 'excel-data-analytics' if d['id'] == 'excel' else d['id']
...
def write_tutorial_pages(root: Path, data: dict, shell) -> list[str]:
    ...
    for path in ['tutorials','exercises','quiz','examples','references','playground/sql']:
