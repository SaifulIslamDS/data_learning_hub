...
from content.datasets.catalog import DATASETS
from content.projects import load_projects
...
def build_content() -> dict:
    ...
    return {
        ...
        "release_roadmap": [
            ...
            {"version": "v2.1.0", "title_en": "Tutorial platform core & complete Data Foundations", "title_bn": "Tutorial platform core ও complete Data Foundations", "status": "completed"},
            {"version": "v2.2.0", "title_en": "Complete Excel for Data Analytics tutorial", "title_bn": "Data Analytics-এর জন্য complete Excel tutorial", "status": "completed"},
            {"version": "v2.3.0", "title_en": "Complete SQL tutorial with browser practice", "title_bn": "Browser practiceসহ complete SQL tutorial", "status": "completed"},
            {"version": "v2.4.0", "title_en": "Complete Power BI tutorial", "title_bn": "Complete Power BI tutorial", "status": "completed"},
            {"version": "v2.5.0", "title_en": "Complete Python for Analytics tutorial", "title_bn": "Complete Python for Analytics tutorial", "status": "completed"},
            {"version": "v2.6.0", "title_en": "Data Analytics workflows & portfolio projects", "title_bn": "Data Analytics workflow ও portfolio project", "status": "current"},
        ],
    }
...
def render_portfolio_project(project: dict) -> str:
    tool_chips = "".join(f'<span>{escape(tool)}</span>' for tool in project["tools"])
    questions = "".join(f'<li>{escape(item)}</li>' for item in project["questions"])
    deliverables = "".join(f'<li>{escape(item)}</li>' for item in project["deliverables"])
    gates = "".join(f'<li>{escape(item)}</li>' for item in project["quality_gates"])
    portfolio = "".join(f'<li>{escape(item)}</li>' for item in project["portfolio_sections"])
    phases = "".join(
        f'<article class="project-phase-card"><label><input type="checkbox" data-project-task="{project["id"]}-{phase["id"]}"><span class="project-phase-number">{phase["id"]}</span><span><strong data-en="{escape(phase["title_en"], quote=True)}" data-bn="{escape(phase["title_bn"], quote=True)}">{escape(phase["title_en"])}</strong><small data-en="{escape(phase["summary_en"], quote=True)}" data-bn="{escape(phase["summary_bn"], quote=True)}">{escape(phase["summary_en"])}</small></span></label></article>'
        for phase in project["workflow"]
    )
    downloads = "".join(
        f'<a class="button primary" href="{escape(item["url"], quote=True)}" download>{escape(item["title_en"])} ↓</a>'
        for item in project["downloads"]
    )
    return f'''
<section class="project-shell portfolio-project-page"><div class="container"><nav class="breadcrumbs" aria-label="Breadcrumb"><a href="/">Home</a><span>/</span><a href="/projects/">Projects</a><span>/</span><span>{escape(project['title_en'])}</span></nav><header class="project-hero"><div><span class="eyebrow">Complete cross-tool portfolio project</span><h1 data-en="{escape(project['title_en'], quote=True)}" data-bn="{escape(project['title_bn'], quote=True)}">{escape(project['title_en'])}</h1><p data-en="{escape(project['summary_en'], quote=True)}" data-bn="{escape(project['summary_bn'], quote=True)}">{escape(project['summary_en'])}</p><div class="project-tools">{tool_chips}</div></div><div class="project-progress-card"><span>{escape(project['level'])} · {project['estimated_hours']} hours</span><strong data-project-progress>0/8 phases</strong><div class="progress-track"><span data-project-progress-bar style="width:0"></span></div><button class="button small ghost" type="button" data-reset-project>Reset progress</button></div></header><div class="project-download-panel"><div><span class="section-kicker">Start package</span><h2>Download data, starter files and portfolio templates</h2><p>All files are synthetic and documented for educational use.</p></div><div class="hero-actions">{downloads}<a class="button ghost" href="/assets/downloads/portfolio/data-analytics-portfolio-toolkit.zip" download>Portfolio toolkit ↓</a></div></div><section class="project-section"><span class="section-kicker">01 · Business brief</span><h2>Questions this project must answer</h2><ul class="objective-list">{questions}</ul></section><section class="project-section"><span class="section-kicker">02 · End-to-end workflow</span><h2>Complete all eight phases</h2><p>Check a phase only after its evidence, files and validation notes are complete.</p><div class="project-workflow-grid">{phases}</div></section><div class="portfolio-section-grid"><section class="project-section"><span class="section-kicker">03 · Deliverables</span><h2>Required analytical outputs</h2><ul class="project-check-list">{deliverables}</ul></section><section class="project-section"><span class="section-kicker">04 · Quality gates</span><h2>Do not publish before these pass</h2><ul class="project-check-list">{gates}</ul></section><section class="project-section"><span class="section-kicker">05 · Portfolio story</span><h2>Recommended README structure</h2><ol class="project-check-list">{portfolio}</ol></section><section class="project-section"><span class="section-kicker">06 · Continue learning</span><h2>Connect workflow and tools</h2><div class="stacked-actions"><a class="button primary" href="/tutorials/data-analytics-workflows/">Open workflow tutorial →</a><a class="button ghost" href="/tutorials/excel-data-analytics/">Excel tutorial</a><a class="button ghost" href="/tutorials/sql-data-analytics/">SQL tutorial</a><a class="button ghost" href="/tutorials/power-bi-data-analytics/">Power BI tutorial</a><a class="button ghost" href="/tutorials/python-data-analytics/">Python tutorial</a></div></section></div></div></section>
'''
...
"""
    ...
    projects_main = """
<section class="page-hero compact"><div class="container"><span class="eyebrow" data-en="Portfolio Project Center" data-bn="Portfolio Project Center">Portfolio Project Center</span><h1 data-en="Build complete analytics projects from business question to portfolio story" data-bn="Business question থেকে portfolio story পর্যন্ত complete analytics project তৈরি করুন">Build complete analytics projects from business question to portfolio story</h1><p data-en="Six documented synthetic cases connect Statistics, Excel, SQL, Power BI and Python through one repeatable workflow, quality gates and decision-ready deliverables." data-bn="ছয়টি documented synthetic case এক repeatable workflow, quality gate ও decision-ready deliverable দিয়ে Statistics, Excel, SQL, Power BI ও Python connect করে।">Six documented synthetic cases connect Statistics, Excel, SQL, Power BI and Python through one repeatable workflow, quality gates and decision-ready deliverables.</p></div></section>
<section class="section"><div class="container"><div id="projects-root"></div></div></section>
"""
    (ROOT / "projects" / "index.html").write_text(html_shell(title=f"Portfolio Projects | {SITE['name']}", description="Six complete cross-tool Data Analytics portfolio projects with synthetic datasets, workflows, quality gates and downloadable starter packages.", page="projects", base="projects/", main_html=projects_main, extra_scripts='<script src="/assets/js/projects.js" defer></script>'), encoding="utf-8")

    for project in data["projects"]:
        project_dir = ROOT / "projects" / project["id"]
        project_dir.mkdir(parents=True, exist_ok=True)
        (project_dir / "index.html").write_text(html_shell(title=f"{project['title_en']} | {SITE['name']}", description=project["summary_en"], page="portfolio-project", base=f"projects/{project['id']}/", body_attrs=f'data-project="{project["id"]}"', main_html=render_portfolio_project(project), extra_scripts='<script src="/assets/js/portfolio-project.js" defer></script>'), encoding="utf-8")
...
<section class="page-hero error-page"><div class="container"><span class="error-code">404</span><h1 data-en="This page is not published." data-bn="এই page publish করা হয়নি।">This page is not published.</h1><p data-en="Use Learn, Practice, Projects or Curriculum to find available and planned content." data-bn="Available ও planned content খুঁজতে Learn, Practice, Projects বা Curriculum ব্যবহার করুন।">Use Learn, Practice, Projects or Curriculum to find available and planned content.</p><div class="hero-actions"><a class="button primary" href="/learn/" data-en="Open Learn" data-bn="Learn খুলুন">Open Learn</a><a class="button ghost" href="/curriculum/" data-en="View curriculum" data-bn="Curriculum দেখুন">View curriculum</a></div></div></section>"""
    ...
    urls = ["/", "/start/", "/my-learning/", "/learn/", "/practice/", "/projects/", "/career-paths/", "/curriculum/", "/glossary/", "/about/"]
    urls += [project["url"] for project in data["projects"]]
